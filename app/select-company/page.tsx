...
// ใช้ Hook และ Event ของ React สำหรับจัดการข้อมูลและฟอร์ม
import { ChangeEvent, FormEvent, useCallback, useEffect, useState } from "react"; 

// Icon สำหรับสถานะ ปุ่ม และการอัปโหลดไฟล์
import { CheckCircle2, Clock3, FileText, FileUp, Loader2, Send, X, XCircle } from "lucide-react"; 
import StudentSidebar from "@/components/StudentSidebar"; 
import { useAuth } from "@/components/AuthProvider"; 
import { supabase } from "@/lib/supabase"; 
 
// กำหนดประเภทสถานะต่าง ๆ ที่ใช้ในระบบ
...
// จำกัดขนาดและชนิดไฟล์หลักฐานที่สามารถอัปโหลดได้
const MAX_EVIDENCE_FILE_SIZE = 10 * 1024 * 1024; 
const EVIDENCE_EXTENSIONS = ["pdf", "doc", "docx", "png", "jpg", "jpeg", "webp"]; 
 
// ฟังก์ชันช่วยดึงข้อความ Error สำหรับแสดงบนหน้า
...
// แปลงขนาดไฟล์จาก Byte เป็น KB หรือ MB
function formatFileSize(bytes: number) { 
  if (bytes < 1024 * 1024) return `${Math.max(1, Math.round(bytes / 1024))} KB`; 
  return `${(bytes / (1024 * 1024)).toFixed(1)} MB`; 
} 
 
// แปลงสถานะจากฐานข้อมูลเป็นข้อความภาษาไทย
...
// กำหนดลำดับสถานะที่นักศึกษาสามารถเปลี่ยนได้
...
// ข้อความสถานะการเลือกบริษัทฝึกงาน
...
export default function SelectCompanyPage() { 
  // State หลักสำหรับข้อมูลผู้ใช้ รายการสมัคร และข้อความแจ้งเตือน
  ...
  // State สำหรับ Modal เปลี่ยนสถานะและไฟล์หลักฐาน
  ...
  // โหลดข้อมูลการสมัครและคำขอบริษัทภายนอกจาก Supabase
  const loadData = useCallback(async () => { 
    ...
    // จัดเตรียมข้อมูลหลังจากโหลดจากฐานข้อมูล
    ...
    // เก็บข้อมูลการสมัครไว้ใน State และเรียงประวัติการอัปเดตล่าสุดก่อน
  ...
  // โหลดข้อมูลเมื่อสถานะ Login พร้อมแล้ว
  useEffect(() => { 
  ...
  // เริ่มติดตามการสมัครของบริษัทภายนอกที่ได้รับอนุมัติ
  ...
  // เปิด Modal เพื่อขอเปลี่ยนสถานะการสมัคร
  ...
  // ตรวจสอบไฟล์หลักฐานก่อนนำไปอัปโหลด
  ...
  // อัปโหลดหลักฐานและส่งคำขอเปลี่ยนสถานะไปตรวจสอบ
  ...
  // เปิดไฟล์หลักฐานผ่าน Signed URL ของ Supabase Storage
  ...
  // ส่งคำขอเลือกบริษัทเป็นสถานที่ฝึกงานให้ Coordinator ยืนยัน
  ...
  // แสดงเฉพาะบริษัทภายนอกที่อนุมัติแล้วและยังไม่ได้เริ่มติดตาม
  ...
  // แสดง Loading ระหว่างรอข้อมูลจาก Supabase
  ...
  return <div className="flex min-h-screen bg-slate-50 text-slate-800"> 
    ...
    <main className="min-w-0 flex-1 p-6 md:p-8"><div className="mx-auto max-w-6xl space-y-7"> 
      {/* ส่วนหัวของหน้าสำหรับแสดงชื่อและคำอธิบาย */} 
        ...
        {applications.length === 0 ? ( 
          ...
          <div className="divide-y divide-slate-100"> 
            {applications.map((application) => { 
              // ตรวจสอบว่ารายการนี้มีคำขอเปลี่ยนสถานะที่กำลังรอตรวจหรือไม่
    ...
    {pendingStatusUpdate && ( 
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={() => busyId !== pendingStatusUpdate.application.id && setPendingStatusUpdate(null)}> 
        <form onSubmit={submitStatusUpdate} onClick={(event) => event.stopPropagation()} className="w-full max-w-lg rounded-lg bg-white shadow-xl"> 
          {/* Modal สำหรับแนบหลักฐานและส่งคำขอเปลี่ยนสถานะ */} 
