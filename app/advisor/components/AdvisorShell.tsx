...
export default function AdvisorShell({ student, active, children, title }: { student?: Student; active: "progress" | "evaluation"; children: ReactNode; title?: string }) {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const links = [
    { label: "ภาพรวมและสถิติ", href: "/advisor", icon: "dashboard" as const, active: false },
    { label: "ความก้าวหน้านักศึกษา", href: "/advisor/students", icon: "users" as const, active: active === "progress" },
    { label: "บันทึกนิเทศและแบบประเมิน", href: "/advisor/evaluations", icon: "checklist" as const, active: active === "evaluation" },
  ];
  return <div className="advisor-app advisor-details">
    ...
    <main className="main-content detail-main">
      <div className="detail-context">
        ...
        <div className="breadcrumb">
          <Link href="/advisor">ภาพรวมและสถิติ</Link>
          <span>›</span>
          <Link href={active === "progress" ? "/advisor/students" : "/advisor/evaluations"}>{title ?? (active === "progress" ? "ความก้าวหน้านักศึกษา" : "บันทึกนิเทศและแบบประเมิน")}</Link>
          {student && <><span>›</span><strong>{student.name}</strong></>}
        </div>{student && <label className="student-picker">นักศึกษา<select value={student.id} onChange={e => router.push(`/advisor/${active === "progress" ? "students" : "evaluations"}/${e.target.value}`)}>{students.map(s => <option value={s.id} key={s.id}>{s.name} ({s.id})</option>)}</select></label>}</div>
