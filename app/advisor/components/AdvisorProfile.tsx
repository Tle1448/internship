...
type Profile = { full_name: string | null; user_code: string | null; email: string | null; phone: string | null; faculty: string | null; major: string | null };
type Draft = { full_name: string; phone: string; faculty: string; major: string };

const display = (value: string | null | undefined) => value?.trim() || "ไม่ระบุ";
const toDraft = (profile: Profile | null): Draft => ({ full_name: profile?.full_name || "", phone: profile?.phone || "", faculty: profile?.faculty || "", major: profile?.major || "" });
...
export default function AdvisorProfile() {
  ...
  async function save() {
    ...
    setSaving(true);
    const payload = { full_name: draft.full_name.trim(), phone: draft.phone.trim() || null, faculty: draft.faculty.trim() || null, major: draft.major.trim() || null };
    const { data, error } = await supabase.from("profiles").update(payload).eq("id", user.id).select("full_name, user_code, email, phone, faculty, major").single();
    setSaving(false);
    if (error) { setMessage(`บันทึกข้อมูลไม่สำเร็จ: ${error.message}`); return; }
    const next = data as Profile;
    setProfile(next); setDraft(toDraft(next)); setEditing(false); setMessage("บันทึกข้อมูลส่วนตัวแล้ว");
  }
