...
<<<<<<< HEAD
  return <AdvisorShell student={student} active="students" studentSection="history">{message && <p className="feedback">{message}</p>}<StudentHeader student={student} compact /><section className="detail-card evaluation-history"><div className="section-title"><span className="detail-icon"><Icon name="clock" /></span><div><h2>Saved evaluations</h2><p>Newest submission first.</p></div></div>{history.length ? history.map((entry) => <div className="attachment-row" key={entry.id}><Icon name="check" /><span>{new Date(entry.saved_at).toLocaleString()}</span><strong>{entry.total_score}/100 · Grade {entry.grade}</strong></div>) : <div className="empty-state"><Icon name="clock" size={28} /><strong>No submitted evaluation yet.</strong></div>}</section></AdvisorShell>;
=======
