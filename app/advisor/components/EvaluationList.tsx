...
export default function EvaluationList() {
  ...
  useEffect(() => { void (async () => {
    ...
    const { data } = await supabase.from("weekly_logs").select("record_id").in("record_id", students.map((student) => student.recordId)).in("status", ["pending", "revision"]);
    setPendingLogs(new Set((data || []).map((row) => row.record_id)));
  })(); }, [students]);

  const tasks = useMemo(() => students.flatMap((student): Task[] => {
    const next: Task[] = [];
    if (pendingLogs.has(student.recordId)) next.push({ id: `${student.recordId}-weekly`, student, kind: "weekly", title: "ตรวจบันทึกประจำสัปดาห์", urgent: student.progressHealth === "attention", href: `/advisor/students/${student.id}/progress`, action: "ตรวจบันทึก" });
  ...
  return <section className="advisor-list-page task-inbox" aria-labelledby="task-list-title">
    ...
    <div className="evaluation-task-list">{pageTasks.map((task) => <article className="detail-card evaluation-task-item" key={task.id}><span className="task-icon"><Icon name={task.kind === "weekly" ? "file" : task.kind === "supervision" ? "calendar" : "checklist"} size={21} /></span><div><span className="task-kind">{task.kind === "weekly" ? "WEEKLY LOG" : task.kind === "supervision" ? "SUPERVISION" : "EVALUATION"}</span><h2>{task.student.name}</h2><p>{task.title}</p><small>{task.student.id} · {task.student.company}</small></div><span className={`badge ${task.urgent ? "late" : "pending"}`}>{task.urgent ? "ต้องติดตาม" : "รอดำเนินการ"}</span><Link className="button primary" href={task.href}>{task.action}<Icon name="chevron" size={16} /></Link></article>)}</div>
