...
export default function UserEditModal({ user, initialId = "", mode = "edit", roles = ["Student", "Coordinator", "Advisor"], onClose, onSave }: Props) {
  ...
  return (
    <dialog ref={dialog} aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); onClose(); }} className="fixed inset-0 m-auto h-[min(800px,calc(100dvh-2rem))] max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-[800px] overflow-hidden rounded-2xl border-0 bg-white p-0 text-[#292934] shadow-2xl backdrop:bg-black/40">
      <div className="flex h-full flex-col">
        <header className="flex shrink-0 items-start gap-3 border-b border-gray-100 bg-[#F5F5F6] px-5 py-6 sm:px-6">
          ...
          <button type="button" onClick={onClose} aria-label="ปิดหน้าต่าง" className="cursor-pointer rounded-md px-2 py-1 text-xl text-gray-500 hover:bg-gray-200">×</button>
        </header>

        <form id={formId} onSubmit={submit} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-6 sm:px-6">
          <label className="block text-xs font-semibold text-gray-600"><span className="flex flex-wrap justify-between gap-2">รหัสประจำตัวระบบ (System UID)<span className="font-mono text-[10px] font-normal tracking-wide text-gray-400">{isCreate ? "AUTO-GENERATED ? READ-ONLY" : "READ-ONLY"}</span></span><input readOnly value={draft.id} className="mt-1.5 w-full rounded-lg border border-transparent bg-[#EEEEEF] px-3 py-2.5 font-mono text-sm font-normal text-gray-500" /></label>
        ...
        <footer className="flex shrink-0 flex-wrap justify-end gap-3 border-t border-gray-100 bg-[#F5F5F6] px-5 py-5 sm:px-6">
          <button type="button" onClick={onClose} className="cursor-pointer rounded-lg border border-gray-200 bg-white px-5 py-3 text-sm font-semibold hover:bg-gray-50">ยกเลิก (Cancel)</button>
          <button type="submit" form={formId} className="flex cursor-pointer items-center gap-2 rounded-lg bg-[#3D348B] px-5 py-3 text-sm font-semibold text-white hover:bg-[#5146AA]"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="size-4"><path d="M5 3h12l4 4v14H3V3h2Z" /><path d="M7 3v6h10V3M7 21v-8h10v8" /></svg>บันทึกการเปลี่ยนแปลง (Save Changes)</button>
        </footer>
      </div>
    </dialog>
  );
